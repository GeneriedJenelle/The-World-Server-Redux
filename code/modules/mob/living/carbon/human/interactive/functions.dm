	...
	if(internalBeaker && internalBag)
		...
		for(var/obj/machinery/portable_atmospherics/hydroponics/tester in view(12,src))
			...
			if(tester.pestlevel > 0)
				considered[tester] += 5
			if(tester.nutrilevel <  10)
				considered[tester] += 15
			if(tester.waterlevel < 100)
				considered[tester] += 15
...
/mob/living/carbon/human/interactive/proc/shitcurity(obj)
	...
	if(retal && TARGET && Adjacent(TARGET))
		var/obj/item/weapon/handcuffs/R = locate() in allContents
		if(R)
...
//	var/static/list/customableTypes = list(/obj/item/weapon/reagent_containers/food/snacks,/obj/item/weapon/reagent_containers/food/snacks/slice/bread,/obj/item/weapon/reagent_containers/food/snacks/bun,/obj/item/weapon/reagent_containers/food/snacks/sliceable/flatdough,/obj/item/weapon/reagent_containers/food/snacks/boiledspagetti,/obj/item/trash/plate,/obj/item/trash/snack_bowl)
	...
	try
		...
		var/obj/item/weapon/material/kitchen/rollingpin/RP = locate() in allContents
		if(!RP)
			RP = new /obj/item/weapon/material/kitchen/rollingpin(src)

		var/obj/item/weapon/material/knife/KK = locate() in allContents
		if(!KK)
			KK = new /obj/item/weapon/material/knife(src)
...
/*
		...
		for(var/customType in customableTypes)
			...
			var/obj/item/weapon/reagent_containers/food/snacks/I = A
		...
		if(SF)
			if(SF.contents.len > 0)
				if(!Adjacent(SF))
					...
					for(var/obj/item/A in SF.item_records)
						if(prob(smartness/2))
						/*
							var/count = SF.item_quants[A.name]
							if(count)
								SF.item_quants[A.name] = count - 1 */
							SF.item_records.get_product(src)
...
/mob/living/carbon/human/interactive/proc/paperwork_sentence()
	...
	return pick(
		...
		"The [adjective_insult] [pick(security_positions)] [past_verb(verbs_touch)] the [adjective_generic] [nouns_insult] [pick(civilian_positions)]'s [nouns_body] and had to be [result].",
