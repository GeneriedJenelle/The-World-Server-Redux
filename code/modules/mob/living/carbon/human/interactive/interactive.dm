...
/mob/living/carbon/human/interactive
	...
	var/default_job = /datum/job/assistant	// the type for the default job
	var/datum/job/myjob
...
/mob/living/carbon/human/interactive/proc/random()
	...
	age = rand(species.min_age, species.max_age)
	...
	myjob.equip(src)
//	myjob.apply_fingerprints_to_item(src)
...
/mob/living/carbon/human/interactive/proc/setup_job(thejob)
	switch(thejob)
		...
		if("Nanotrasen Representative")
			favoured_types = list(/obj/item/clothing, /obj/item/weapon/stamp/centcom, /obj/item/weapon/paper, /obj/item/weapon/melee/baton)
			functions += "paperwork"
		if("Judge", "CBIA Agent")
			favoured_types = list(/obj/item/clothing, /obj/item/weapon/stamp/law, /obj/item/weapon/paper)
			functions += "paperwork"
		if("Factory Manager", "Factory Worker")
			favoured_types = list(/obj/item/clothing, /obj/item/weapon/stamp/cargo, /obj/item/weapon/stamp/denied, /obj/item/weapon/paper, /obj/item/weapon/clipboard)
			functions += "stamping"
		if("Chef")
			favoured_types = list(/obj/item/weapon/reagent_containers/food, /obj/item/weapon/material/kitchen)
			functions += "souschef"
			restrictedJob = 1
		if("Bartender")
			favoured_types = list(/obj/item/weapon/reagent_containers/food,/obj/item/weapon/reagent_containers/food/drinks, /obj/item/weapon/material/kitchen)
			functions += "bartend"
			restrictedJob = 1
		if("Firefighter", "Chief Engineer", "City Engineer")
		...
		if("Chief of Police", "Prison Warden", "Police Officer", "Detective")
			favoured_types = list(/obj/item/clothing, /obj/item/weapon, /obj/item/weapon/handcuffs)
		if("Janitor")
...
/mob/living/carbon/human/interactive/proc/makeTraitor(var/inPers)
	...
	for(var/datum/antagonist/traitor/W)
...
//	ticker.mode.update_traitor_icons_added(mind)
	...
	switch(traitorType)
		...
		if(SNPC_STEALTH) // Shhh we is sneekies
			...
			S.find_target()
			mind.objectives += S
			traitorTarget = locate(S.steal_target.type) in world
		if(SNPC_MARTYR) // MY LIFE FOR SPESZUL
			var/targetType = pick(/obj/structure/particle_accelerator, /obj/machinery/power/smes)
...
/mob/living/carbon/human/interactive/attackby(obj/item/I, mob/living/user, def_zone)
	..()
	retalTarget(user)
